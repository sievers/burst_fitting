...
if ~exist('dat')
  ...
  crap=load('chains/chain_qu_real_newdat.txt_1');crap=crap(round(0.2*end):end,3:end);
  best_guess=mean(crap);best_guess(end+1)=0.2;
  mycov=cov(crap);mycov(end+1,end+1)=0.05^2;

   
end

myopts.nstep=500000;
myopts.outroot='chains/chain_qu_ramp_real_newdat_test.txt';
myopts.func=@get_burst_real_chisq_qu_ramp_cached_c;
%myopts.func=@get_burst_real_chisq_qu_cached_c;
