...
if ~exist('dat')
  ...
  crap=read_chains('chains/chain_tt_kiyo_tau_test_try2.txt',0.2,'');crap=crap(:,3:end);
end
...
best_guess=mean(crap);
mycov=cov(crap);
%best_guess(end+1)=1e-4;
%mycov(end+1,end+1)=mycov(2,2);


myopts.nstep=3e5;
myopts.outroot='chains/chain_tt_kiyo_tau_wcov.txt';
myopts.func=@get_burst_chisq_tau_cached_c;
...
for j=1:length(pvec),
  ...
  chivec(j)=get_burst_chisq_tau_cached_c(gg,dataptr,freq_use,length(dat),nvec,dt,imin,model);
end
