...
if ~exist('dat')
  ...
  %crap=load('chains/chain_tt_real_test.txt_1.org');crap=crap(round(0.2*end):end,3:end);
  crap=read_chains('chains/chain_tt_real_test3.txt',0.2,'');crap=crap(:,3:end);


  best_guess=mean(crap);best_guess(end+1)=-3.7;
  ...
  mycov=cov(crap);mycov(end+1,end+1)=1.2^2;
...
myopts.nstep=500000;
myopts.outroot='chains/chain_tt_real_scatpow_test.txt';
