...
crap=read_chains('chain_polfit_rmpow.txt',0.2,'');
cc=crap(:,2:end);best_guess=mean(cc);
cc=cc-repmat(best_guess,[length(cc) 1]);
mycov=cc'*cc/length(cc);
mycov=0.5*(mycov+mycov');
sigs=std(cc);


%sigs(end+1)=0.03;
%mycov(end+1,end+1)=sigs(end)*sigs(end);
%best_guess(end+1)=2+2*randn(1)*sigs(end);

[pp,ll,qft,uft,nvec]=mcmc_burst_polfit_rmpow(dat_q_use,dat_u_use,freq_use,best_guess,mycov*0.5,dt,1.0);
